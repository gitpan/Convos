...
#     META_MERGE => { resources=>{ repository=>q[https://github.com/Nordaaker/convos], homepage=>q[http://convos.by], bugtracker=>q[https://github.com/Nordaaker/convos/issues], license=>q[http://www.opensource.org/licenses/artistic-license-2.0] } }
#     NAME => q[Convos]
#     PREREQ_PM => { Parse::IRC=>q[1.18], Mojo::IRC=>q[0.0303], Unicode::UTF8=>q[0.58], Mojolicious::Plugin::AssetPack=>q[0.06], Test::More=>q[0.90], Mojolicious=>q[4.6], IRC::Utils=>q[0.12], Time::Piece=>q[1.2], Mojo::Redis=>q[0.992] }
#     TEST_REQUIRES => {  }
...
PM_TO_BLIB = lib/Convos/public/font/fontawesome-webfont.woff \
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'version: 0.3001' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '      },' >> META_new.json
	$(NOECHO) $(ECHO) '      "homepage" : "http://convos.by",' >> META_new.json
	$(NOECHO) $(ECHO) '      "license" : [' >> META_new.json
	...
	$(NOECHO) $(ECHO) '      }' >> META_new.json
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "version" : "0.3001"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
ppd :
	...
	$(NOECHO) $(ECHO) '        <REQUIRE VERSION="0.992" NAME="Mojo::Redis" />' >> $(DISTNAME).ppd
